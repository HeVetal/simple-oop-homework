...
public class TaskStream {
    ...
    public static boolean task7(List<Book> books) {
        ...
        return books.stream()
                .map(Book::getAuthor)
                .allMatch(author -> author.contains("Автор"));
    }
    ...
    public static List<Book> task9(List<Book> books) {
        ...
        return books.stream().filter(book -> book.getPrice() < 100)
    ...
    public static Map<String, List<Book>> task10(List<Book> books) {
        return books.stream().collect(Collectors.groupingBy(book -> book.getPrice() < 50? "OK" : "Not Ok"));
    }
    ...
    public static List<Book> task11(List<Book> books) {
        return books.stream().filter(book -> book.getReviews().stream().anyMatch(string -> string.toLowerCase().contains("рекомендую"))).toList();
